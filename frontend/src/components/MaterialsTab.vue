...
const loadMaterials = async () => {
  try {
    // ONLY load generic materials for Settings page
    const response = await APIService.getMaterials({ type: 'generic' })
...
const saveItem = async () => {
  ...
  const data = {
    ...
    is_generic: true, // CRITICAL: Materials from Settings are always generic types
  }
  try {
    if (isEditing) {
