...
const colorMode = ref('single')
const lowStockEnabled = ref(true) // Enable/disable low stock alerts
...
const isAdditionalPhotoModalVisible = ref(false)
const selectedAdditionalPhoto = ref(null)
...
const loadMaterial = async () => {
  try {
    ...
    material.value = {
      ...
      colors: data.colors && data.colors.length > 0 ? data.colors : ['#000000'],
    }
    photoPreview.value = data.photo
    // Set color mode based on number of colors
    colorMode.value = data.colors && data.colors.length > 1 ? 'multi' : 'single'
    // Set low stock enabled based on whether threshold exists
...
const loadAdditionalPhotos = async () => {
  ...
  try {
    const response = await axios.get(`/api/materials/${route.params.id}/photos/`)
    additionalPhotos.value = response.data
  } catch (error) {
...
const uploadAdditionalPhoto = async () => {
  if (!additionalPhotoFile.value || !route.params.id) return

  isUploadingPhoto.value = true
  try {
    const formData = new FormData()
    formData.append('image', additionalPhotoFile.value)
    if (newPhotoCaption.value) {
      formData.append('caption', newPhotoCaption.value)
    }

    await axios.post(`/api/materials/${route.params.id}/photos/`, formData, {
...
const deleteAdditionalPhoto = async (photoId) => {
  if (!confirm('Are you sure you want to delete this photo?')) return

  try {
    await axios.delete(`/api/material-photos/${photoId}/`)
    await loadAdditionalPhotos()
  } catch (error) {
...
const saveMaterial = async (redirectToSpool = false) => {
  try {
    ...
    if (!material.value.is_generic) {
      ...
      if (material.value.empty_spool_weight)
        formData.append('empty_spool_weight', material.value.empty_spool_weight)
    ...
    await axios.patch(`/api/materials/${route.params.id}/`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })

    if (redirectToSpool) {
      // Redirect to spool creation with this material pre-selected
      router.push(`/filaments/create?materialId=${route.params.id}`)
    } else {
      // Go back to material detail view
      router.push(`/filaments/materials/${route.params.id}`)
    }
  } catch (error) {
  ...
  <div>
    ...
    <form @submit.prevent="saveMaterial(false)" class="material-form">
      <div class="form-section">
        ...
        <div class="info-box">
          <p>
            Type: <strong>{{ material.is_generic ? 'Generic Material' : 'Blueprint' }}</strong>
          </p>
          <p class="note">Material type cannot be changed after creation.</p>
        </div>
      </div>
      ...
      <div class="form-section">
        ...
        <div
          v-if="!material.is_generic && route.params.id"
          class="form-group additional-photos-section"
        >
          <label>Additional Photos</label>

          <!-- Existing Additional Photos -->
          <div v-if="additionalPhotos.length > 0" class="additional-photos-grid">
            <div v-for="photo in additionalPhotos" :key="photo.id" class="additional-photo-item">
              <img
                :src="photo.image"
              ...
              <div class="photo-overlay">
                ...
                <button
                  ...
                  @click="deleteAdditionalPhoto(photo.id)"
                  title="Delete photo"
                >
                  ×
                </button>
          ...
          <div class="add-photo-inline">
            ...
            <button
              ...
              :disabled="!additionalPhotoFile || isUploadingPhoto"
            >
              {{ isUploadingPhoto ? 'Uploading...' : 'Add Photo' }}
            </button>
          </div>
          <p class="help-text">
            Add extra photos of this material (packaging, test prints, swatches, etc.)
          </p>
        </div>
      </div>
      ...
      <div v-if="!material.is_generic" class="form-section">
        ...
        <div class="form-group">
          ...
          <input type="number" id="spool-weight" v-model.number="material.spool_weight" />
        </div>
      ...
      <div v-if="!material.is_generic" class="form-section">
        ...
        <div class="form-group">
          ...
          <input type="url" id="vendor-link" v-model="material.vendor_link" />
        </div>

        <div class="form-group">
          <label for="price">Price Per Spool</label>
          <input type="number" id="price" v-model.number="material.price_per_spool" step="0.01" />
        </div>
      </div>
      ...
      <div v-if="!material.is_generic" class="form-section">
        ...
        <div v-if="lowStockEnabled" class="form-group">
          ...
          <input type="number" id="low-stock" v-model.number="material.low_stock_threshold" />
        </div>
      ...
      <div class="form-actions">
        <button
          type="button"
          @click="router.push(`/filaments/materials/${route.params.id}`)"
          class="btn-cancel"
        >
          Cancel
        </button>
    ...
    >
      <div class="modal-content" @click.stop>
        ...
        <img
          ...
          :src="selectedAdditionalPhoto.image"
...
.additional-photo-thumb.clickable {
  cursor: pointer;
  transition:
    opacity 0.2s ease,
    transform 0.2s ease;
...
.additional-photo-thumb.clickable:hover {
  opacity: 0.9;
